...
/*
  ...
  Version v10.0p_2015-01-31/LGPL Development Only (2015-01-31)
...
if (window.isc && isc.version != "v10.0p_2015-01-31/LGPL Development Only") {
    isc.logWarn("SmartClient module version mismatch detected: This application is loading the core module from "
        + "SmartClient version '" + isc.version + "' and additional modules from 'v10.0p_2015-01-31/LGPL Development Only'. Mixing resources from different "
...
//> @attr calendar.monthMoreEventsLinkTitle (string : "+ \${eventCount} more..." : IR)
...
removeButtonDefaults: {
    click : function () {
        this.calendar.removeEvent(this.calendar.eventEditorLayout.event);
            ...
            isc.DynamicForm.create({
                ...
                submit : function () {
                    ...
                    if (cal.isTimeline() || (cal.dayViewSelected() && cal.showDayLanes)) {
                        lane = dataForm.getItem(cal.laneNameField).getValue();
                        sublane = dataForm.getItem(cal.sublaneNameField).getValue();
                    }
    ...
    this.eventEditor = this.createAutoChild("eventEditor", {
        ...
        initWidget : function () {
            ...
            if (editStyle == "date" || editStyle == "datetime") {
                ...
                for (var key in arr) {
                    var width = cal.measureText(arr[key], 45);
        ...
        getTimeValues : function (type, startTime) {
            ...
            ;
            if (type == "hours") {
                var count = cal.twentyFourHourTime ? 24 : 12;
                for (var i = startTime; i < count; i++) {
                    // stringify the hours
                    var stringHour = i < 10 ? "0" + (i) : "" + (i);
                    obj[i + ""] = stringHour;
                }
    ...
    this.eventEditorLayout = this.createAutoChild("eventEditorLayout", {
        ...
        setDate : function (startDate, endDate, eventName, lane, sublane) {
            ...
            if (editStyle == "date" || editStyle == "datetime") {
                ...
                var sTime = isc.Time.toTime(startDate, cal.timeFormatter, true),
                    eTime = isc.Time.toTime(endDate, cal.timeFormatter, true)
                ;
                form.getItem("startHours").setValue(parseInt(sTime.substring(0, sTime.indexOf(":"))));
                form.getItem("endHours").setValue(parseInt(eTime.substring(0, eTime.indexOf(":"))));
                form.getItem("startMinutes").setValue(parseInt(sTime.substring(sTime.indexOf(":") + 1, sTime.indexOf(":") + 3)));
                form.getItem("endMinutes").setValue(parseInt(eTime.substring(eTime.indexOf(":") + 1, eTime.indexOf(":") + 3)));
        ...
        setEvent : function (event) {
            ...
            if (!cal.twentyFourHourTime) {
                form.getItem("startAMPM").show();
                form.getItem("endAMPM").show();
            } else {
                form.getItem("startAMPM").hide();
                form.getItem("endAMPM").hide();
            }
